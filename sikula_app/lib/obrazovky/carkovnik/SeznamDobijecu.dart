...
class SeznamDobijecu extends StatelessWidget {
  SeznamDobijecu({super.key});
  ...
  Future<List<Konzument>> _vratSerazeneKonzumenty() async {
    ...
    seznamKonzumentu.sort((a, b) {
      int aKusy = a.nacarkovanychKusu ?? 0; // Pokud je null, použij 0
      int bKusy = b.nacarkovanychKusu ?? 0; // Pokud je null, použij 0
      return bKusy.compareTo(aKusy); // Sestupné pořadí
    });
    int poradi = 1;
    for (int i = 0; i < seznamKonzumentu.length; i++) {
      if (i > 0 &&
          seznamKonzumentu[i].nacarkovanychKusu ==
              seznamKonzumentu[i - 1].nacarkovanychKusu) {
          ...
          Expanded(
            child: SeznamBezSubtitle(
                ...
                trailingFce: (Konzument konzument) =>
                    konzument.nacarkovanychKusu.toString(),
